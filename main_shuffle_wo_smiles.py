...
def Eval(model, dataloader, device, args):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader, desc="Processing batches", unit="batch"):
            ...
            text_rep = model.text_encoder(orig_text)
...
def main(args):
    ...
    trainset_path="/public/home/weixy2024/momunew/data/chebi20train"
    devset_path="/public/home/weixy2024/momunew/data/chebi20test"
    testset_path="/public/home/weixy2024/momunew/data/chebi20test"
    
    ids = []
    text_name_list = os.listdir(trainset_path+"/text")
    ...
    scaf = []
    scaf.append(seq)
    TrainSet = GINMatchShuffleDataset(args,ids, scaf[0], trainset_path)
    print("len_TrainSet:",len(TrainSet))
    
    
    ids = []
    text_name_list = os.listdir(devset_path+"/text")
    for text_name in text_name_list:
        text_id = re.split('[_.]',text_name)[1]
        text_id = int(text_id)
        ids.append(text_id)
    ...
    scaf = []
    scaf.append(seq)
    DevSet = GINMatchShuffleDataset(args, ids, scaf[0], devset_path)
    print("len_DevSet:",len(DevSet))
    
    
    ids = []
    text_name_list = os.listdir(testset_path+"/text")
    for text_name in text_name_list:
        text_id = re.split('[_.]',text_name)[1]
        text_id = int(text_id)
        ids.append(text_id)
    ...
    scaf = []
    scaf.append(seq)
    TestSet = GINMatchShuffleDataset(args, ids, scaf[0], testset_path)
    print("len_TestSet:",len(TestSet))
    ...
    if args.mode != 'zeroshot': # finetune
        for epoch in range(args.epoch):
            ...
            logging.info('Epoch: %d, DevAcc1: %.4f', epoch, acc1)
            logging.info('Epoch: %d, DevAcc2: %.4f', epoch, acc2)
            ...
            logging.info('Epoch: %d, Acc: %.4f, Loss: %.4f', epoch, acc / allcnt, sumloss / allcnt)

        acc1, acc2 = Eval(model, dev_dataloader, device, args)
        #print('Epoch:', args.epoch, ', DevAcc1:', acc1)
        #print('Epoch:', args.epoch, ', DevAcc2:', acc2)
        
        logging.info('Epoch: %d, DevAcc1: %.4f', args.epoch, acc1)
        logging.info('Epoch: %d, DevAcc2: %.4f', args.epoch, acc2)
        
        if acc1>best_acc:
            best_acc = acc1
            torch.save(model.state_dict(), f'{args.output_path}/model.ckpt')
            #print('Save checkpoint ', global_step)
            logging.info('Save checkpoint %d', global_step)
    ...
    if args.data_type == 'para': # para-level
        ...
        logging.info('Test Acc1: %.4f', round(acc1, 4))
        logging.info('Test Acc2: %.4f', round(acc2, 4))
